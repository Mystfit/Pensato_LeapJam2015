...
public class PensatoGrabbingHand : MonoBehaviour
{
    ...
    // Maximum distance we can be from a dock to drop a grabable
    public float dockDistance = 2.0f;
    ...
    protected IDockable active_closest_dockable_;
    ...
    protected Collider FindClosestComponent<T>(Vector3 pinch_position, float distance, Func<T, bool> checkValidComponent = null)
    {
        Collider closest = null;
        float closest_sqr_distance = distance * distance;
        Collider[] close_things =
            Physics.OverlapSphere(pinch_position, distance, grabbableLayers);
        ...
        for (int j = 0; j < close_things.Length; ++j)
        {
            ...
            {
                T grabbable = close_things[j].GetComponent<T>();
                bool valid = true;
                if (checkValidComponent != null) valid = (grabbable != null) ? checkValidComponent(grabbable) : false;
                if (valid)
                {
    ...
    protected void ContinueHardPinch()
    {
        ...
        //Find the closest dropzone and let it know our coordinates for visualization
        Func<IDockable, bool> isDockable = x => x.IsDockable;
        Collider closestCollider = FindClosestComponent<IDockable>(current_pinch_position_, dockDistance, isDockable);
